...
void VladE::init1() {
	...
	dealtDamageToPlayer = new bool[GlobalRecources::players->size()];
...
void VladE::execute1() {
	for (int i = 0; i < projectileCount; i++) {
		projectiles[i]->move(GlobalRecources::worldHeight, GlobalRecources::worldWidth, GlobalRecources::terrain->getCollidables());
	}



	for (int j = 0; j < GlobalRecources::players->size(); j++) {
		for (int i = 0; i < projectileCount; i++) {
